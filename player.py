...
class Player(pygame.sprite.Sprite):
    ...
    def __init__(self, player_img, shoot_sound, shield_up_sound, shield_down_sound, powerup_sound, bomb_sound):
        super().__init__()
        self.image_orig = player_img # Store the original image passed from main
        ...
        self.bomb_count = 0 # Start with 0 bombs

        # Shield Visuals
        self.shield_visual_radius = max(self.rect.width, self.rect.height) // 2 + 8
        self.shield_visual_color = SHIELD_VISUAL_COLOR # Use color from settings

        # Sounds (Store references passed from main)
    ...
    def shoot(self):
        ...
        if now - self.last_shot_time > self.shoot_delay:
            ...
            return new_bullets # Return list of new bullets for main.py to handle
    ...
    def update(self):
        ...
        self.image = self.image_orig.copy() # Start with the base image
        if self.shield_active:
            # Create a temporary surface for the shield effect
            shield_surf = pygame.Surface(self.image.get_size(), pygame.SRCALPHA)
            pygame.draw.circle(shield_surf, self.shield_visual_color,
                               shield_surf.get_rect().center, self.shield_visual_radius)
            self.image.blit(shield_surf, (0, 0)) # Draw shield effect onto player image
    ...
    def use_bomb(self, enemies_group):
        ...
        if self.bomb_count > 0:
            ...
            for enemy in enemies_group.sprites():
                 # Check if it's a regular enemy (not the boss) before killing
                 # This assumes Boss is not in the 'enemies' group passed here
                 # If Boss *could* be in this group, add a check: if not isinstance(enemy, EnemyBoss):
                 enemy.kill()
                 killed_count += 1
                 # Score increase is handled per enemy in main loop usually,
                 # but bombs grant score immediately.
                 score_increase += 1
            ...
            print("没有炸弹可用!")
        return score_increase # Return how much score increased